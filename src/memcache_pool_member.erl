...
idle(Req = #req{}, State = #state{name = Name}) ->
    memcache_pool_member:return(Name, Req),
    {ok, idle, State}.
...
setup_connection(State = #state{session_id = 0}) ->
    ...
    case gen_tcp:send(Socket,
                      memcache_protocol:connect(Timeout,SessionId,Passwd)) of
        {error, _} -> update_seq(State);
        ok ->
            case memcache_protocol:connect_response(Socket) of

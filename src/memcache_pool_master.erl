...
init(#pool_spec{}) ->
    ...
    {ok, Max} = application:get_env(memcache, max_queue_len),
...
handle_req(Req = #req{}, State) ->
    ...
    NewState =
        case select(Req1, State) of
            State1 = #state{} -> State1;
            {Member, State1} ->
                memcache_pool_member:request(Member, Req1),
                State1
        end,
    {ok, NewState}.
...
flush(State = #state{queue = Queue = [H | _]}) ->
    ...
    [memcache_pool_member:request(Member, Req) || Req <- Queue],
    State1#state{queue = [], queue_len = 0}.

info(Format, Args) -> memcache_log:info(?MODULE, Format, Args).

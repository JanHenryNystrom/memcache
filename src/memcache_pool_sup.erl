...
child(master, Spec) ->
    {master, {memcache_pool_master, start, [Spec]},
     permanent, ?MASTER_SHUTDOWN, worker, [memcache_pool_master]};
child(supervisor, Spec) ->
    {sup, {memcache_pool_members_sup, start_link, [Spec]},
     permanent, infinity, supervisor, [master_members_sup]}.
